...
第 1 步 · 看金样并锁定课型
引导页已打开（index.local.html / index.html）：
· 点开金样卡片，看关键页预览和「金样内容组成」（例如绿色单品是金银花露真实 5 页结构）。
· 用「复制金样内容组成 / 确认选用 · 复制给 WorkBuddy」粘贴给我；或直接用自然语言说交付物、主题和现有资料。
· 不确定选哪个时，直接说用途和主题，我会对照已签样金样推荐；你确认课型前，我不会建任务。
· 不需要下载或填写 Word。

当前 PPT 能力（金样优先）：
· 绿色商品培训 5 页、疾病+商品场景 18 页、商品培训课件3 13 页、成分健康科普 20 页，都是一等高保真固定课型。
· 自由构件化不在业务货架展示，仅作金样缺页的内部补漏；不引导业务自由拼页。
...
第 4 步 · 补素材并确认
包装图、Logo、标签和证据由你提供授权原件；其他教学/场景插图由我按模板固定图槽生成并绑定。我先锁定内容，再给你看代表图和全部插图的真实图槽效果，最后绑定你授权的包装原图。

第 5 步 · 生成、逐页质检与取件
确认齐全后，我生成正式 PPTX/MP4，完成质检才放入交付目录。要改就说「第二页…改成…」；内容或图片变化后会重新确认再出一版。
...
现在可以直接告诉我自然语言需求，或在门户复制金样内容组成后粘贴给我。我会先按金样结构解释课型；你确认后才开始整理初稿。
```
...
| 1 | 门户复制金样内容组成 / 直接发需求 | 运行 `business_job.py recommend --text '<自然语言需求>'`；返回 1 个推荐，歧义时仅 2 个候选 + 1 个追问；明确 active/环境边界，不创建任务 |
| 2 | 确认模板 | 锁定已签样 settled 金样；金样没有的页签由系统内部补漏，不引导业务自由拼构件；一套课件只锁一个 style pack |
