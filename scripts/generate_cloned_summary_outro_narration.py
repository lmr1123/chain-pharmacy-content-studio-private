#!/usr/bin/env python3
"""Generate paragraph-level narration for the wind-heat summary replica."""
...
OUTPUT_DIR = Path(
    "poc/reference-replica/reference-analysis/audio/qwen-clone-summary-outro-v1"
)
FINAL_OUTPUT = Path(
    "poc/gold-sample/public/reference-audio/qwen-cloned-summary-outro-v1.wav"
)
TIMING_OUTPUT = Path(
    "poc/gold-sample/public/reference-audio/"
    "qwen-cloned-summary-outro-v1-timing.json"
)
TIMELINE_DURATION = 24.54
CROSSFADE_SECONDS = 0.035

BLOCKS = [
    {
        "id": "opening",
        "text": "最后，给大家划个重点，记不住的可以多看两遍。",
    },
    {
        "id": "cause",
        "text": "病因，风加热一起入侵，肺气不顺。",
    },
    {
        "id": "symptoms",
        "text": "症状，发热，口渴，喉咙痛，咳黄痰，流黄涕，心烦。",
    },
    {
        "id": "care",
        "text": "调理，疏风清热，用桑叶、菊花、薄荷，清淡饮食，多喝水。",
    },
    {
        "id": "avoidance",
        "text": "禁忌，辛辣刺激，烟酒，温补燥热食物。",
    },
]
...
def main() -> None:
    ...
    mastered, mastered_rate = sf.read(FINAL_OUTPUT, dtype="float32")
    mastered_target = round(TIMELINE_DURATION * mastered_rate)
    if len(mastered) < mastered_target:
        mastered = np.pad(mastered, (0, mastered_target - len(mastered)))
        sf.write(FINAL_OUTPUT, mastered, mastered_rate)
