...
const projectFile = path.resolve(projectRoot, 'src/sufuda-editable-project.tsx');
...
const server = await createServer({
  ...
  plugins: [
    ...
    revideo({project: projectFile, rendererPlayer: true}),
  ],
  server: {port, host: '127.0.0.1', strictPort: true},
  resolve: {
    alias: {
      '@revideo/2d': path.resolve(goldSampleRoot, 'node_modules/@revideo/2d'),
      '@revideo/core': path.resolve(goldSampleRoot, 'node_modules/@revideo/core'),
      '@revideo/ui': path.resolve(goldSampleRoot, 'node_modules/@revideo/ui'),
    },
  },
});

await server.listen();
console.log(`Sufuda courseware editor: http://127.0.0.1:${port}/`);
console.log(`State dir: ${validationDir}`);
console.log('Export PPTX uses content-model + patches; video re-renders full gold timeline.');
