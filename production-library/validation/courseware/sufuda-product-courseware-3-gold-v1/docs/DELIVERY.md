...
## 金样归档（2026-08-02 用户确认）

正式沉淀目录（**按商品名命名，便于以后辨认**）：

`production-library/templates/settled/sufuda-mabaloshawei-product-courseware-3-v1/`

| 交付物 | 文件名 |
| --- | --- |
| 视频金样 | `速福达玛巴洛沙韦_商品培训课件3_金样_v1.mp4` |
| 可编辑 PPT | `速福达玛巴洛沙韦_商品培训课件3_金样_可编辑课件_v1.pptx` |
| 注册表 | `template.sufuda-mabaloshawei-product-courseware-3-v1`（`registries/templates.json`） |

工程目录仍为本路径（含源码/编辑器）；**签样成片以 settled 为准**。

### 业务整片预览

| 页 | 路径 |
| --- | --- |
| 本金样整片预览 | `web/full-film.html` |
| 金样案例汇总（后续逐个点进） | `../gold-samples/index.html` |
...
# http://127.0.0.1:8765/sufuda-product-courseware-3-gold-v1/web/full-film.html
...
| 项 | 值 |
| --- | --- |
| 归档路径 | `templates/settled/sufuda-mabaloshawei-product-courseware-3-v1/速福达玛巴洛沙韦_商品培训课件3_金样_v1.mp4` |
| 工作区副本 | `out/速福达玛巴洛沙韦_商品培训课件3_金样_v1.mp4` |
| PPT 归档 | `…/速福达玛巴洛沙韦_商品培训课件3_金样_可编辑课件_v1.pptx`（13 页） |
| 分辨率 | 1920×1080 |
| 帧率 | 30 fps |
| 编码 | H.264 + AAC |
| 时长 | **约 94 s**（签样成片） |
...
- **已**于 2026-08-02 写入 settled 金样目录（见文首「金样归档」）  
- 不与 `product-courseware-3-gold-sample-v1/v2`、辅酶Q10 faithful、风热证金样共用输出目录  
- 不绑定 `template.product-training-faithful-v1` 蓝底人物模板  

## 内容模型 · PPTX · 编辑器 · 换主题（2026-08-02）

| 能力 | 入口 |
|------|------|
| 内容模型 | `content-model.json`（与视频/PPTX 共用） |
| 元素清单 | `layer-manifest.json`（136 个稳定 `editable:sufuda:*`） |
| 可编辑 PPTX | `npm run export:pptx` → `out/速福达玛巴洛沙韦_商品培训课件3_金样_可编辑课件_v1.pptx` |
| 业务编辑器 | `npm run start:editor` → http://127.0.0.1:9010/ |
| 换主题 | `python3 scripts/replicate_courseware_theme.py --theme theme-packages/<包> --skip-tts` |
| 说明 | `docs/content-model-and-editor.md` |

克隆旁白音色包：`voice.sufuda-courseware-pharmacist-v1`（量产换主题用）。
...
4. 真实新主题：填 `theme-packages/_blank` 或提供 Word 后映射 theme.json 再 `replicate_courseware_theme.py`
