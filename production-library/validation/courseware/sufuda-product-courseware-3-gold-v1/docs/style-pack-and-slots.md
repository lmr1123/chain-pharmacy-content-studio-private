...
1. 复制本工程时间轴与 chrome（章节带 / 导航 / 字幕系统）——或直接用 `replicate_courseware_theme.py`  
2. 业务填 `theme-packages/_blank/theme.json`（审核文案 + 授权图路径）；系统回填 `content-model.json`  
3. 仅补本课语义所需插画槽位 + 授权包装  
4. `npm run export:pptx` 出可编辑课件；克隆旁白后 `npm run render` 出 MP4  
5. 局部微调：`npm run start:editor`（`editable:sufuda:*` 图层）

元素清单：`layer-manifest.json`。说明：`docs/content-model-and-editor.md`。
