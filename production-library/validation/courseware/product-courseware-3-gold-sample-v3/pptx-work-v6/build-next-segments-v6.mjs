...
const WORK = path.join(ROOT, "pptx-work-v6");
const RENDER = path.join(ROOT, "pptx-render-v6-next");
const OLD_ALPHA = path.join(ROOT, "assets", "alpha");
const V5_ALPHA = path.join(ROOT, "assets", "v5-alpha");
const V6_ALPHA = path.join(ROOT, "assets", "v6-alpha");
const BG = path.join(ROOT, "assets", "background", "white-silk-reference-faithful-bg-v3.png");
const FINAL = path.join(ROOT, "商品培训课件3_精细复刻_PPTX继续片段_v6.pptx");
...
async function build() {
  ...
  const [bg, packs, tablets, bottles, elder, chronic, childSafety, safetyShield, speechBubble, silhouette, schoolgirl] = await Promise.all([
    bytes(BG),
    ...
    bytes(path.join(V6_ALPHA, "child-safety-alpha.png")),
    bytes(path.join(V6_ALPHA, "safety-shield-alpha.png")),
    bytes(path.join(V6_ALPHA, "speech-bubble-alpha.png")),
    bytes(path.join(V6_ALPHA, "person-silhouette-alpha.png")),
    bytes(path.join(V6_ALPHA, "schoolgirl-alpha.png")),
  ]);
...

  {
    ...
    image(slide, childSafety, "儿童安全与低呕吐率精制透明图标", {left: 48, top: 260, width: 260, height: 260}, "child-safety-icon");
    text(slide, "呕吐率低", {left: 52, top: 488, width: 252, height: 62}, {fontSize: 39, color: C.navy, name: "low-vomiting-label"});
    image(slide, safetyShield, "精制安全盾牌透明图标", {left: 995, top: 295, width: 190, height: 190}, "safety-shield-icon");
    text(slide, "安全性良好", {left: 955, top: 485, width: 270, height: 58}, {fontSize: 34, color: C.navy, name: "safety-label"});
...

  {
    ...
    image(slide, tablets, "四种无品牌白色片剂与刻度尺透明插画", {left: 70, top: 320, width: 275, height: 220}, "tablets-layer");
    image(slide, packs, "无品牌包装组合透明插画", {left: 435, top: 305, width: 410, height: 230}, "dosage-package-slot");
    image(slide, bottles, "无品牌干混悬剂药瓶透明插画", {left: 960, top: 305, width: 225, height: 285}, "suspension-layer");
    ...
    image(slide, speechBubble, "无字奶油橙色气泡壳", {left: 62, top: 220, width: 290, height: 200}, "tablet-bubble-shell");
    text(slide, "片剂无味\n药小易吞", {left: 100, top: 255, width: 215, height: 92}, {fontSize: 27, color: C.navy, name: "tablet-bubble-text"});
    image(slide, speechBubble, "无字奶油橙色气泡壳", {left: 940, top: 230, width: 255, height: 175}, "suspension-bubble-shell");
    text(slide, "草莓口味", {left: 976, top: 276, width: 185, height: 54}, {fontSize: 29, color: C.navy, name: "suspension-bubble-text"});
    text(slide, "可按体重精准给药", {left: 928, top: 555, width: 290, height: 40}, {fontSize: 21, color: C.navy, name: "weight-dose-label"});
...

  {
    ...
    image(slide, silhouette, "年龄中性人物轮廓透明插画", {left: 245, top: 215, width: 280, height: 410}, "audience-silhouette-intro");
    ...
    notes(slide, "01:11.92–01:17.36", "先建立抽象人物轮廓和5岁以上门槛；轮廓不是最终页具体儿童，文字独立可编辑。");
  }

  {
    ...
    const cards = [
      ...
      {left: 430, width: 330, img: schoolgirl, pos: {left: 500, top: 225, width: 190, height: 320}, label: "学龄期儿童\n（5岁及以上）", name: "audience-schoolgirl"},
    ...
    cards.forEach((card) => {
      ...
      text(slide, card.label, {left: card.left + 15, top: 548, width: card.width - 30, height: 58}, {fontSize: card.name === "audience-chronic" ? 22 : 24, color: C.navy, name: `${card.name}-label`});
    });
