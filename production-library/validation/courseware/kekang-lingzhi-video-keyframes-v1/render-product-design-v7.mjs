...
try {
  ...
  await page.goto(
    pathToFileURL(path.join(here, 'product-course-design-v7.html')).href,
    {waitUntil: 'networkidle0'},
  );
  await page.evaluate(() => document.fonts.ready);

  for (let index = 1; index <= 6; index += 1) {
    const slide = await page.$(`#v7-slide-${index}`);
    if (!slide) throw new Error(`Missing #v7-slide-${index}`);
    await slide.screenshot({
      path: path.join(
        here,
        `product-course-v7-${String(index).padStart(2, '0')}.png`,
      ),
      type: 'png',
    });
  }
