...
// engines/product-courseware-green-v1 → repo root (3 levels up)
const repoDir = path.resolve(workDir, "../../..");
// --data / --out / --qa：默认本引擎金样 JSON；业务任务通过 --data 注入主题稿。
function cliValue(flag) {
  const index = process.argv.indexOf(flag);
  return index >= 0 && process.argv[index + 1] ? path.resolve(process.argv[index + 1]) : null;
}
const dataPath =
  cliValue("--data") ?? path.join(workDir, "gold-content-model.json");
const outputPptx =
  cliValue("--out") ??
  path.join(
    repoDir,
