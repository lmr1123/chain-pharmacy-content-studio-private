...
// --out <pptx路径> / --qa <QA目录>：默认保持 settled 输出，行为不变；
// 升级迭代时改道 validation 目录，避免误覆盖已签样金样。
...
// 品牌绿色阶
const GREEN = "#009900"; // 主色（大参林绿，不变）
const GREEN_DEEP = "#066A2F"; // 标题/重强调
...
function rect(slide, name, x, y, width, height, fill = WHITE, line = "none", lineWidth = 0, opts = {}) {
...
function addChrome(slide, number, title) {
  ...
  if (number) {
    rect(slide, "chrome-left-number", 32, 18, 64, 42, GREEN_DEEP, "none", 0, { radius: 10 });
    text(slide, "chrome-page-number", number, 32, 18, 64, 42, {
      size: 21,
      color: WHITE,
      bold: true,
      align: "center",
  ...
  line(slide, "chrome-hairline", 32, 73, 1216, 1, LINE, 0);
  rect(slide, "chrome-rule-accent", 114, 70, 72, 5, GREEN, "none", 0, { radius: 2 });
  text(slide, "chrome-brand", "大参林 dashenlin", 1080, 18, 164, 40, {
    size: 14,
    color: GREEN_2,
    bold: true,
    align: "right",
  });
  text(slide, "chrome-internal-notice", "仅限于内部学习", 1010, 688, 160, 22, {
    size: 12,
    color: MUTED,
    align: "right",
  });
  text(slide, "chrome-page-index", `${String(pageNo).padStart(2, "0")} / 18`, 1180, 688, 64, 22, {
    size: 12,
    color: MUTED,
    align: "right",
  });
...
async function addLocalImage(slide, name, fileName, frame, alt, fit = "cover", opts = {}) {
  slide.images.add({
  ...
  if (opts.frame) {
    rect(slide, `${name}-frame`, frame.left, frame.top, frame.width, frame.height, "none", LINE, 1);
  }
}

function card(slide, name, x, y, width, height, titleValue, bodyValue, options = {}) {
  const {
    titleColor = GREEN_DEEP,
    fill = WHITE,
    border = LINE,
    titleSize = 23,
    bodySize = 20,
  ...
  rect(slide, `${name}-surface`, x, y, width, height, fill, border, 1, { radius: 12 });
  rect(slide, `${name}-accent`, x + 10, y + 14, 4, 26, GREEN, "none", 0, { radius: 2 });
  text(slide, `${name}-title`, titleValue, x + 24, y + 12, width - 40, 38, {
    size: titleSize,
    color: titleColor,
    bold: true,
    vAlign: "middle",
  });
  line(slide, `${name}-divider`, x + 24, y + 58, width - 44, 1, LINE, 0);
  text(slide, `${name}-body`, bodyValue, x + 24, y + 70, width - 44, height - 84, {
    size: bodySize,
    color: GRAY,
    bold: bodyBold,
    emphasis,
...
function callout(slide, name, value, x, y, width, height, options = {}) {
  const { fill, color = WHITE, size = 18, bold = true, emphasis = [] } = options;
  // 默认薄荷底+绿左条+深字（总结条场景）；显式 fill 保留原语义，仅统一圆角。
  ...
  const short = height <= 72;
  text(slide, name, value, x + insetX, short ? y : y + 10, width - insetX - 14, short ? height : height - 20, {
    size,
    color: textColor,
    bold,
    vAlign: short ? "middle" : undefined,
    emphasis,
  });
}
...
function simpleCell(slide, name, value, x, y, width, height, options = {}) {
  ...
  text(slide, `${name}-text`, value, x + 8, y, width - 16, height, {
    size,
    color,
    bold,
    align,
...
// 3. Agenda.
{
  ...
  line(slide, "agenda-rule", 625, 175, 520, 2, LINE, 0);
  const agenda = [
    "疾病篇 — 风热证",
    "商品篇 — 穿心莲内酯滴丸的核心优势",
    "场景篇 — 什么时候推荐穿心莲内酯滴丸",
  ...
  agenda.forEach((item, index) => {
    ...
    ellipse(slide, `agenda-chip-${index + 1}`, 625, y + 4, 40, 40, GREEN);
    text(slide, `agenda-chip-num-${index + 1}`, `0${index + 1}`, 625, y + 4, 40, 40, {
      size: 17,
      color: WHITE,
      bold: true,
      align: "center",
    ...
    text(slide, `agenda-item-${index + 1}`, item, 685, y, 560, 48, {
      size: 23,
      color: INK,
      bold: true,
      vAlign: "middle",
  ...
  text(slide, "agenda-internal-notice", "仅限于内部学习", 1010, 688, 160, 22, { size: 12, color: MUTED, align: "right" });
  text(slide, "agenda-page-index", "03 / 18", 1180, 688, 64, 22, { size: 12, color: MUTED, align: "right" });
...
// 5. Symptom gallery.
{
  ...
  for (const [index, item] of imageFrames.entries()) {
    await addLocalImage(slide, `symptom-image-${index + 1}`, item.fileName, { left: item.x, top: 112, width: 320, height: 185 }, item.alt, "cover", { frame: true });
  }
...
// 6. Wind-heat vs wind-cold.
{
  ...
  function comparisonPanel(x, titleValue, color, fill, leftBody, rightBody, bottomBody) {
    rect(slide, `${titleValue}-surface`, x, 105, 550, 360, WHITE, LINE, 1, { radius: 12 });
    rect(slide, `${titleValue}-header`, x + 1, 106, 548, 67, fill, "none", 0, { radius: 12 });
    rect(slide, `${titleValue}-header-square`, x + 1, 140, 548, 33, fill);
    text(slide, `${titleValue}-title`, titleValue, x + 30, 120, 490, 44, { size: 26, color: INK, bold: true, vAlign: "middle" });
  ...
  text(slide, "change-note", "病情演变：清涕转黄涕，应紧扣当前主症及时调整治法，不拘泥于初期寒象。", 70, 620, 1140, 42, { size: 20, color: INK, bold: true, fill: "#FFF3C4", radius: 10, vAlign: "middle" });
  addNotes(slide, 6);
}
...
// 9. Product information.
{
  ...
  rect(slide, "product-info-outline", 360, 135, 835, 440, WHITE, GREEN_DEEP, 1.5, { radius: 12 });
  const rows = [
  ...
  rows.forEach(([label, value], index) => {
    text(slide, `product-info-label-${index + 1}`, label, 400, 170 + index * 62, 170, 38, { size: 22, bold: true, color: GREEN_DEEP });
...
// 10. One mechanism, three advantages.
{
  ...
  ellipse(slide, "advantage-core", 535, 105, 210, 150, GREEN_DEEP);
  text(slide, "advantage-core-text", "一机理\n三优势", 555, 140, 170, 82, { size: 28, color: WHITE, bold: true, align: "center" });
  const nodes = [
    { x: 130, color: GREEN_DEEP, title: "机理", heading: "抑制核转录因子表达", body: "提高机体防御能力\n减轻细菌或病毒感染带来的不适症状" },
    { x: 400, color: GREEN, title: "单", heading: "百分百纯植物提取", body: "单体制剂、植物化学药\n机理确切可控，降低联合用药风险" },
    { x: 680, color: "#2E9E5B", title: "纯", heading: "穿心莲内酯纯度97%", body: "质量稳定，载药量高\n疗效好" },
    { x: 960, color: GREEN_2, title: "快", heading: "滴丸剂型固体分散技术", body: "溶散时间短\n快速吸收，起效迅速" },
  ];
  line(slide, "advantage-axis", 220, 365, 830, 2, LINE, 0);
  nodes.forEach((node, index) => {
    ...
    text(slide, `advantage-heading-${index + 1}`, node.heading, node.x - 65, 425, 205, 58, { size: 19, color: GREEN_DEEP, bold: true, align: "center" });
...
// 11. Core advantages summary.
{
  ...
  card(slide, "formula-advantage", 810, 220, 430, 300, "组方优势", "• 植物单体制剂：经典老药，植物单体制剂\n• 功能全面：清热解毒＋抗菌消炎\n• 疗效显著：治疗感冒、流感、咽痛总有效率95%", {
    titleSize: 26,
    bodySize: 22,
    bodyBold: false,
...
// 12. Audience scenarios.
{
  ...
  callout(slide, "audience-label-child", "儿童", 355, 225, 76, 48, { size: 25, fill: GREEN });
  text(slide, "audience-arrow-child-1", "→", 600, 214, 78, 48, { size: 42, color: GREEN_2, bold: true, align: "center" });
  text(slide, "audience-child-scene", "学校感冒交叉感染", 700, 215, 220, 48, { size: 23, color: BLACK, bold: true, align: "center" });
  text(slide, "audience-arrow-child-2", "→", 915, 214, 70, 48, { size: 42, color: GREEN_2, bold: true, align: "center" });
  text(slide, "audience-child-need", "酸奶送服更安全", 995, 215, 210, 48, { size: 22, color: "#6E7378", bold: true });

  callout(slide, "audience-label-adult", "中青年", 395, 368, 104, 48, { size: 25, fill: GREEN });
  text(slide, "audience-arrow-adult-1", "→", 625, 372, 78, 48, { size: 42, color: GREEN_2, bold: true, align: "center" });
  const adultRows = [
    ["办公室白领", "办公室空调干燥易咽痛"],
    ["商旅人士", "出差出行劳累抵抗力低"],
    ["老师", "课堂授课易咽炎"],
  ...
  adultRows.forEach(([person, need], index) => {
    ...
    text(slide, `audience-arrow-adult-${index + 1}`, "→", 865, y - 3, 65, 42, { size: 38, color: GREEN_2, bold: true, align: "center" });
    text(slide, `audience-adult-need-${index + 1}`, need, 935, y, 275, 38, { size: 21, color: "#777777", bold: true });
  });

  callout(slide, "audience-label-senior", "老年", 355, 550, 76, 48, { size: 25, fill: GREEN });
  text(slide, "audience-arrow-senior-1", "→", 600, 552, 78, 48, { size: 42, color: GREEN_2, bold: true, align: "center" });
  text(slide, "audience-senior-scene", "秋冬呼吸\n疾病高发", 705, 525, 160, 88, { size: 22, color: BLACK, bold: true, align: "center" });
  text(slide, "audience-arrow-senior-2", "→", 865, 552, 65, 42, { size: 38, color: GREEN_2, bold: true, align: "center" });
...
// 13. Consultation framework.
{
  ...
  text(slide, "consultation-thesis", "实战销售话术：以“望闻问切”辨顾客，以专业方案做推荐", 70, 115, 1100, 48, { size: 25, color: GREEN_DEEP, bold: true });
...
// 14. Scenario 1.
{
  ...
  text(slide, "scenario1-title", "场景1：顾客主诉“喉咙痛”", 80, 90, 600, 45, { size: 25, color: GREEN_DEEP, bold: true });
...
// 15. Scenario 2.
{
  ...
  text(slide, "scenario2-title", "场景2：顾客主诉“感冒了，发烧，有黄痰”", 80, 90, 750, 45, { size: 25, color: GREEN_DEEP, bold: true });
  ...
  callout(slide, "scenario2-talktrack", "关键话术：“您这是风热感冒，穿心莲内酯滴丸清除热毒是治本，复方氨酚烷胺片缓解症状是治标，一个治标一个治本，一起用效果更好。”", 80, 570, 1120, 65, { size: 20, fill: GREEN });
  addNotes(slide, 15);
}
...
// 17. Weighted product detail.
{
  ...
  simpleCell(slide, "weighted-code-h", "编码", 45, 425, 90, 48, { fill: GREEN_DEEP, color: WHITE, bold: true });
  simpleCell(slide, "weighted-priority-h", "主推", 135, 425, 90, 48, { fill: GREEN_DEEP, color: WHITE, bold: true });
  simpleCell(slide, "weighted-spec-h", "规格", 225, 425, 160, 48, { fill: GREEN_DEEP, color: WHITE, bold: true });
  simpleCell(slide, "weighted-price-h", "零售价", 385, 425, 125, 48, { fill: GREEN_DEEP, color: WHITE, bold: true });
  ...
  callout(slide, "weighted-slogan", "一句话卖点（员工宣传语）：\n安宫鼻祖，胆红素含量高于同品", 45, 555, 465, 75, { fill: PALE, color: RED, size: 17 });
  ...
  const tx = 585;
  const ty = 295;
  simpleCell(slide, "bilirubin-h1", "品种", tx, ty, 260, 44, { fill: GREEN_DEEP, color: WHITE, bold: true });
  simpleCell(slide, "bilirubin-h2", "牛黄", tx + 260, ty, 120, 44, { fill: GREEN_DEEP, color: WHITE, bold: true });
  simpleCell(slide, "bilirubin-h3", "胆红素含量", tx + 380, ty, 210, 44, { fill: GREEN_DEEP, color: WHITE, bold: true });
  [["安宫牛黄丸（广誉远）", "天然", "46.6mg/丸"], ["北京同仁堂安宫", "天然", "40.1mg/丸"], ["安宫国家标准", "天然", "≥18.5mg/丸"]].forEach((row, index) => {
    const y = ty + 44 + index * 38;
    simpleCell(slide, `bilirubin-${index + 1}-1`, row[0], tx, y, 260, 38, { fill: index === 1 ? PALE : WHITE, size: 16, color: index === 0 ? RED : BLACK, bold: index === 0 });
    simpleCell(slide, `bilirubin-${index + 1}-2`, row[1], tx + 260, y, 120, 38, { fill: index === 1 ? PALE : WHITE, size: 16, color: index === 0 ? RED : BLACK, bold: index === 0 });
    simpleCell(slide, `bilirubin-${index + 1}-3`, row[2], tx + 380, y, 210, 38, { fill: index === 1 ? PALE : WHITE, size: 16, color: index === 0 ? RED : BLACK, bold: index === 0 });
  });
...
// 18. Weighted product comparison.
{
  ...
  simpleCell(slide, "compare-dim", "对比维度", x, y, widths[0], 66, { fill: GREEN_DEEP, color: WHITE, bold: true, size: 18 });
  simpleCell(slide, "compare-product-a", "安宫牛黄丸（白云山中一）\n8202533（人工体培）", x + widths[0], y, widths[1], 66, { fill: GREEN_DEEP, color: WHITE, bold: true, size: 17 });
  simpleCell(slide, "compare-product-b", "安宫牛黄丸（宏济堂）1185781（单天然）、\n1185783（人工体培）2373089（单天然包金衣）", x + widths[0] + widths[1], y, widths[2], 66, { fill: GREEN_DEEP, color: WHITE, bold: true, size: 16 });
  simpleCell(slide, "compare-image-label", "产品图片", x, y + 66, widths[0], 100, { fill: MINT, bold: true });
  ...
  simpleCell(slide, "compare-selling-label", "核心卖点", x, y + 166, widths[0], 170, { fill: MINT, bold: true, size: 18 });
  ...
  simpleCell(slide, "compare-audience-label", "适宜人群", x, y + 336, widths[0], 85, { fill: MINT, bold: true, size: 18 });
  simpleCell(slide, "compare-audience", "1. 高血压、中风病史等心脑血管高风险人群家庭备用；\n2. 用于中风（热闭证）及高热神昏的急救。", x + widths[0], y + 336, widths[1] + widths[2], 85, { fill: PALE, size: 19, align: "left" });
  simpleCell(slide, "compare-combo-label", "联合用药", x, y + 421, widths[0], 150, { fill: MINT, bold: true, size: 18 });
  simpleCell(slide, "compare-combo", "1. 针对三高人群：按内部审核话术说明节气前备用逻辑；\n2. 针对急救：出现疑似急症应立即呼叫急救并遵循专业人员指导；\n3. 小儿高热：必须严格按内部审核和专业人员指导使用。\n严格辨证：仅用于“热闭”，绝对禁用于“寒闭”和“脱证”。", x + widths[0], y + 421, widths[1] + widths[2], 150, { fill: MINT, color: RED, size: 18, align: "left" });
  addNotes(slide, 18, [
...
// artifact-tool 不回写字体名，PowerPoint 会回退 Calibri —— 统一注入中文字体
const fontPatch = spawnSync(
  "python3",
  [path.join(workDir, "patch-pptx-font.py"), outputPptx, "Microsoft YaHei"],
  { stdio: "inherit" },
);
if (fontPatch.status !== 0) throw new Error("patch-pptx-font.py failed");
