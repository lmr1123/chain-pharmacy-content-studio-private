...
const projectFile = path.resolve(
  projectRoot,
  'src/product-q10-editable-project.tsx',
);
...
const validationDir = path.resolve(
  repositoryRoot,
  'production-library/validation/revideo-editability/q10',
);
...
const port = Number(process.env.REVIDEO_EDITOR_PORT ?? 9001);
...
function startExport(format, patches) {
  ...
  writeJson(snapshotFile, {
    export_id: exportId,
    project_id: 'product.q10.editable-v1',
...
await server.listen();
console.log(`Q10 product-training business editor: http://127.0.0.1:${port}`);
